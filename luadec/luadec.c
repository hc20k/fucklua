...
static void usage(const char* message, const char* arg) {
	...
	fprintf(stderr,
		"Fucklua " VERSION_STRING " for " LUA_VERSION "\n"
		" Credits: Hisham Muhammad, Viruscamp, Zsolt Sz. Sztupak, hpluto888\n"
...
int main(int argc, char* argv[]) {
	...
	for (i=0; i<argc; i++) {
		...
		// check 4 bytes from 0x04 to see if it's a luac header
		if (buffer && buffer_size > 8 && buffer[4] == 0x1B && buffer[5] == 0x4C && buffer[6] == 0x75 && buffer[7] == 0x61) {
			printf("-- Parsing from possible MiHoYoBinData\n");
			// cut first 4 bytes
			buffer += 4;
			buffer_size -= 4;
		}
		...
		if (luaL_loadbuffer(L, buffer, buffer_size, filename) != 0)
	...
	if (disassemble) {
		printf("; Disassembled using fucklua " VERSION_STRING " for " LUA_VERSION ". Lol @mihoyo\n");
		printf("; Command line: ");
	} else {
		printf("-- Decompiled using fucklua " VERSION_STRING " for " LUA_VERSION ". Lol @mihoyo\n");
		if (func_check) {
